...
local Custom = {} do
    -- iOS Dark Mode Inspired Palette
    Custom.ColorRGB = Color3.fromRGB(10, 132, 255)   -- System Blue
    Custom.AccentColor = Color3.fromRGB(0, 122, 255) -- Standard Blue
    Custom.DarkBlue = Color3.fromRGB(28, 28, 30)     -- Secondary System Background
...
function CircleClick(Button, X, Y)
    task.spawn(function()
        -- Simpan warna dan transparency asli
        local originalBgColor = Button.BackgroundColor3
        local originalBgTransparency = Button.BackgroundTransparency
        
        -- Flash putih/abu-abu dengan mengubah background button langsung
        Button.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        Button.BackgroundTransparency = math.max(0, originalBgTransparency - 0.15) -- Lebih terang
        
        local Time = 0.3
        
        -- Animasi kembali ke warna asli
        local ButtonTween = TweenService:Create(Button, TweenInfo.new(Time, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
function Speed_Library:CreateWindow(Config)
    ...
    do 
      ...
      Custom:Create("UICorner", { CornerRadius = UDim.new(0, 2) }, Toggle)
      Custom:Create("UIPadding", {PaddingBottom = UDim.new(0, 10)}, Toggle)
    ...
    do
      local Button = Custom:Create("Frame", {
        ...
        Custom:Create("UICorner", { CornerRadius = UDim.new(0, 2) }, Button)
        Custom:Create("UIPadding", {PaddingBottom = UDim.new(0, 10)}, Button)
  ...
  local SpeedHubXGui = Custom:Create("ScreenGui", {
    ...
    Custom:Create("ImageLabel", {
        ...
        Name = "IconSHIELD",
        ZIndex = 20,
        Parent = Main -- Parented to Main to move with window, but ZIndex > Main
    }, Main)
    ...
    Open_Close.Activated:Connect(function()
        ...
        TweenService:Create(DropShadowHolder, TweenInfo.new(0), {
            Size = UDim2.new(0, 115 + TextLabel.TextBounds.X + 1 + TextLabel1.TextBounds.X, 0, 350)
        }):Play()
    ...
    local LayersReal = Custom:Create("Frame", {
        ...
        Size = UDim2.new(1, 0, 1, -48), -- Adjusted for larger header
        Name = "LayersReal"
    }, Layers)
    ...
    function Tabs:CreateTab(Config)
        ...
        ScrolLayersLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
             ScrolLayers.CanvasSize = UDim2.new(0,0,0, ScrolLayersLayout.AbsoluteContentSize.Y + 10)
        end)
        ...
        TabButton.Activated:Connect(function()
            ...
            if FrameChoose and Tab.LayoutOrder ~= LayersPageLayout.CurrentPage.LayoutOrder then
                ...
                FrameChoose.Position = UDim2.new(0, 3, 0, 12 + (37 * Tab.LayoutOrder))
    
                LayersPageLayout:JumpToIndex(Tab.LayoutOrder)
    
                NameTab.Text = _Name
    
                FrameChoose.Size = UDim2.new(0, 2, 0, 22)
                task.wait(0.05)
                FrameChoose.Size = UDim2.new(0, 2, 0, 16)
            end
        end)
        ...
        function Sections:AddSection(Title, OpenSection)
            ...
            Custom:Create("UICorner", {
                CornerRadius = UDim.new(0, 2) -- Sharp Section
            }, SectionReal)
            
            -- Removed UIGradient for simpler look
            ...
            local SectionTitle = Custom:Create("TextLabel", {
                ...
                Text = Title,
                TextColor3 = Color3.fromRGB(255, 255, 255),
                TextStrokeTransparency = 1, -- Removed visible stroke
                TextSize = 15,
                TextXAlignment = Enum.TextXAlignment.Left,
                TextYAlignment = Enum.TextYAlignment.Center,
                BackgroundColor3 = Color3.fromRGB(255, 255, 255),
      ...
      local function UpdateSizeSection()
        if OpenSection then
          local SectionSizeYWitdh = SectionListLayout.AbsoluteContentSize.Y
  
          TweenService:Create(FeatureFrame, TweenInfo.new(0), {Rotation = 90, BackgroundTransparency = 0}):Play()
          TweenService:Create(Section, TweenInfo.new(0), {Size = UDim2.new(1, 1, 0, SectionSizeYWitdh + 44)}):Play()
          TweenService:Create(SectionAdd, TweenInfo.new(0), {Size = UDim2.new(1, 0, 0, SectionSizeYWitdh)}):Play()
          TweenService:Create(SectionDecideFrame, TweenInfo.new(0), {Size = UDim2.new(1, 0, 0, 3)}):Play()
        end
      end
      ...
      local function ToggleSection()
        ...
        if OpenSection then
            -- Tutup section
            TweenService:Create(FeatureFrame, TweenInfo.new(0), {Rotation = 0}):Play()
            TweenService:Create(Section, TweenInfo.new(0), {Size = UDim2.new(1, 1, 0, 30)}):Play()
            TweenService:Create(SectionDecideFrame, TweenInfo.new(0), {Size = UDim2.new(0, 0, 0, 2)}):Play()
            ...
            TweenService:Create(FeatureFrame, TweenInfo.new(0), {Rotation = 90}):Play()
            TweenService:Create(Section, TweenInfo.new(0), {Size = UDim2.new(1, 1, 0, SectionSizeYWitdh + 38)}):Play()
            TweenService:Create(SectionAdd, TweenInfo.new(0), {Size = UDim2.new(1, 0, 0, SectionSizeYWitdh)}):Play()
            TweenService:Create(SectionDecideFrame, TweenInfo.new(0), {Size = UDim2.new(1, 0, 0, 2)}):Play()
        end
    end
...
function Item:AddParagraph(Config)
    ...
    Custom:Create("UIPadding", {PaddingBottom = UDim.new(0, 10)}, Paragraph)
...
function Item:AddToggle(Config)
    ...
    local function Animate(Value, Instant)
        if Mode == "Toggle" then
            local TitleColor = Value and Custom.ColorRGB or Color3.fromRGB(231, 231, 231)
            ...
            if Instant then
                ...
                if ToggleCircle then ToggleCircle.Position = CirclePosition end
                UIStroke.Color = StrokeColor
                UIStroke.Transparency = StrokeTransparency
                FeatureFrame.BackgroundColor3 = FrameColor
                FeatureFrame.BackgroundTransparency = FrameTransparency
            else
                local tweenInfo = TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
                TweenService:Create(ToggleTitle, tweenInfo, {TextColor3 = TitleColor}):Play()
                if ToggleCircle then TweenService:Create(ToggleCircle, tweenInfo, {Position = CirclePosition}):Play() end
            ...
            if Instant then
                 FeatureFrame.BackgroundColor3 = FrameColor
                 FeatureFrame.BackgroundTransparency = Transparency
            else
                TweenService:Create(FeatureFrame, TweenInfo.new(0), {BackgroundColor3 = FrameColor, BackgroundTransparency = Transparency}):Play()
            end
        end
    end
   ...
   function Item:AddSlider(Config)
        ...
        Custom:Create("UIPadding", {PaddingBottom = UDim.new(0, 10)}, Slider)
...
function Item:AddDropdown(Config)
        ...
        for _, Drop in pairs(ScrollSelect:GetChildren()) do
            if Drop.Name == "Option" then
                ...
                if optionText then
                    ...
                    local tweenInfoInOut = TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
    ...
    function Funcs_Dropdown:AddOption(OptionName)
        ...
        -- Removed UICorner to make it a list style
